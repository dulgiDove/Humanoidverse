...
### WebSocket 최적화
- `send_robot_state()`: 관절각도, 위치, 방향만 매 스텝 전송
- `send_static_state()`: 목표/장애물 위치는 변경 시에만 전송
- Unity 접속 시 초기 목표/장애물 위치 자동 전송

### locomotion.py 개선
- 목표/장애물 배치 기준을 절대 좌표(0,0) 기준으로 변경
- 장애물 배치 시 로봇, 목표, 다른 장애물과 겹침 방지 (최소 0.8m)
- 목표 배치 시 장애물과 겹침 방지
